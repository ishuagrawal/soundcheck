...
Soundcheck finds apps through public Core Audio process objects and `NSWorkspace`. Each app you adjust gets a private process tap and aggregate device for each output device and stream. Muting fades the app out, switches the same tap to `muted`, which silences the app with no reader, and stops the stream, so the hardware can idle. Unmuting restarts the stream, hands the mute back to the reader once it's reading, and fades in. A muted app that isn't running gets only a `muted` tap, so it stays muted when it opens. While an app is audible, its tap uses `mutedWhenTapped`: Core Audio silences the original only while the reader runs, so if Soundcheck stops, the app's own audio returns. A small C callback copies Float32 audio with an atomic gain target and a 5 ms ramp; it doesn't allocate memory, take locks, or call Swift on the audio thread.
